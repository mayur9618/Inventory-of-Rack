...
// ============================================================
// CONSTANTS — ACO 39 Drawer Cabinet Layout
// Each rack: 6 rows of SMALL drawers (S1-S6, 6 wide) + 3 rows of BIG drawers (B1-B3, 3 wide)
// ============================================================

// Small drawer rows: S1..S6, columns 1..6  => S1-1, S1-2 ... S6-6 (36 drawers)
// Big drawer rows:   B1..B3, columns 1..3  => B1-1, B1-2, B1-3 (9 drawers)
// Total = 39 drawers × 2 racks = 78 slots

const SMALL_ROWS = ["S1","S2","S3","S4","S5","S6"];
const SMALL_COLS = [1,2,3,4,5,6];
const BIG_ROWS   = ["B1","B2","B3"];
const BIG_COLS   = [1,2,3];

function makeBlocks(rackNum) {
  const blocks = [];
  SMALL_ROWS.forEach(r => SMALL_COLS.forEach(c => blocks.push(`R${rackNum}-${r}-${c}`)));
  BIG_ROWS.forEach(r => BIG_COLS.forEach(c => blocks.push(`R${rackNum}-${r}-${c}`)));
  return blocks;
}

const ALL_BLOCKS = [...makeBlocks(1), ...makeBlocks(2)];
...
export default function App() {
  ...
  const fetchParts = useCallback(async () => {
    if (isSetup) {
      // demo data
      setParts([
        { part_id: 1, part_name: "Servo Motor SG90", category: "Mechanical", quantity: 12, block_location: "R1-S1-1", notes: "Mini servo" },
        { part_id: 2, part_name: "ESP32 Dev Board", category: "Electronics", quantity: 5, block_location: "R1-S1-2", notes: "" },
        { part_id: 3, part_name: "Ultrasonic Sensor", category: "Sensors", quantity: 8, block_location: "R1-S3-4", notes: "HC-SR04" },
        { part_id: 4, part_name: "M3 Bolts 10mm", category: "Fasteners", quantity: 200, block_location: "R1-B1-1", notes: "" },
        { part_id: 5, part_name: "Stepper Driver A4988", category: "Electronics", quantity: 6, block_location: "R2-S1-1", notes: "" },
        { part_id: 6, part_name: "PVC Tube 20mm", category: "Mechanical", quantity: 3, block_location: "R2-B3-3", notes: "1m lengths" },
      ]);
      setLoading(false);
      return;
    }
  ...
  // ---------- save part ----------
  const savePart = async (formData) => {
    const payload = { ...formData };
    delete payload.photo_url;

    // isEditing only when part has a real saved part_id
    ...
    if (isSetup) {
      ...
      show(isEditing ? "Part updated ✓" : "Part added ✓");
      setModalOpen(false);
      setEditPart(null);
      return;
    }
    ...
    if (isEditing) {
      ...
      // optimistic update so rack reflects immediately
      setParts((prev) => prev.map((p) => (p.part_id === editPart.part_id ? { ...p, ...payload } : p)));
      show("Part updated ✓");
    } else {
      const result = await supabase.post("inventory", payload);
      // optimistic insert — use real id from response if available
      const newPart = Array.isArray(result) && result[0] ? result[0] : { ...payload, part_id: Date.now() };
      ...
      <div className="app">
        ...
        <header className="header">
          ...
          <div className="header-status">
            <div className="dot" />
            {isSetup ? "DEMO MODE" : "LIVE"} &nbsp;·&nbsp; {parts.length} PARTS &nbsp;·&nbsp; {occupiedBlocks}/78 DRAWERS
          </div>
        </header>
          ...
          {tab === "rack" && !selectedBlock && (
            <>
              <div className="stats-row">
                ...
                <div className="stat-card">
                  <div className="stat-label">Occupied Drawers</div>
                  <div className="stat-value">{occupiedBlocks}</div>
                  <div className="stat-sub">of 78 total</div>
                </div>
                <div className="stat-card">
                  <div className="stat-label">Empty Drawers</div>
                  <div className="stat-value" style={{ color: G.muted }}>{78 - occupiedBlocks}</div>
              ...
              <div style={{ display: "flex", gap: 24, flexWrap: "wrap" }}>
                {[1, 2].map((rackNum) => (
                  <AcoRack key={rackNum} rackNum={rackNum} blockCounts={blockCounts} maxCount={maxCount} onSelect={setSelectedBlock} />
                ))}
              </div>
            </>
          )}
...
// ============================================================
// ACO 39-DRAWER RACK COMPONENT
// Layout: 6 rows × 6 small drawers (top) + 3 rows × 3 big drawers (bottom)
// ============================================================
function AcoRack({ rackNum, blockCounts, maxCount, onSelect }) {
  return (
    <div style={{ flex: "1 1 340px" }}>
      <div style={{
        fontFamily: "'Barlow Condensed', sans-serif",
        fontWeight: 800, fontSize: 16, letterSpacing: 3,
        color: "#c8f04a", marginBottom: 10, textTransform: "uppercase"
      }}>
        ▣ Rack {rackNum}
        <span style={{ color: "#7a8a7d", fontWeight: 400, fontSize: 12, marginLeft: 10 }}>
          ACO 39-Drawer Cabinet
        </span>
      </div>
      ...
      <div style={{
        ...
        borderRadius: 8,
        padding: "10px 8px",
        boxShadow: "0 0 24px rgba(245,200,66,0.08)",
      }}>

        {/* SMALL drawers — 6 rows × 6 cols */}
        <div style={{ marginBottom: 6 }}>
          <div style={{ fontSize: 9, color: "#7a8a7d", letterSpacing: 1, marginBottom: 4, paddingLeft: 2 }}>
            SMALL DRAWERS (15.3 × 6.2 × 3.7 cm)
          </div>
          {SMALL_ROWS.map((row) => (
            <div key={row} style={{ display: "grid", gridTemplateColumns: "repeat(6,1fr)", gap: 4, marginBottom: 4 }}>
              {SMALL_COLS.map((col) => {
                const id = `R${rackNum}-${row}-${col}`;
                ...
                return (
                  <div
                    ...
                    style={{
                      height: 38,
                      background: count > 0 ? "#162018" : "#111413",
                      border: `1px solid ${count > 0 ? "#2d4030" : "#2a2f2d"}`,
                      borderRadius: 3,
                      cursor: "pointer",
                      display: "flex",
                    ...
                    onMouseEnter={e => {
                      e.currentTarget.style.borderColor = "#c8f04a";
                      e.currentTarget.style.background = count > 0 ? "#1c2a1e" : "#151918";
                    }}
                    onMouseLeave={e => {
                      e.currentTarget.style.borderColor = count > 0 ? "#2d4030" : "#2a2f2d";
                      e.currentTarget.style.background = count > 0 ? "#162018" : "#111413";
                    }}
                  >
                    <div style={{
                      fontFamily: "'Barlow Condensed',sans-serif",
                      fontWeight: 800, fontSize: 9,
                      color: count > 0 ? "#c8f04a" : "#7a8a7d",
                      letterSpacing: 0.5, lineHeight: 1,
                    }}>{id.replace(`R${rackNum}-`,"")}</div>
                    {count > 0 && (
                      <div style={{ fontSize: 8, color: "#7aaa7d" }}>{count}</div>
                    )}
                    {count > 0 && (
                      <div style={{
                        position: "absolute", bottom: 0, left: 0,
                        height: 2, width: `${pct}%`, background: "#c8f04a",
                      }} />
                    )}
                    {/* drawer handle */}
                    <div style={{
                      position: "absolute", bottom: 3, right: 3,
                      width: 8, height: 2, background: "#2a2f2d",
                      borderRadius: 1,
                    }} />
                  </div>
                );
        ...
        {/* divider */}
        <div style={{ borderTop: "2px solid #f5c842", margin: "6px 0", opacity: 0.4 }} />

        {/* BIG drawers — 3 rows × 3 cols */}
        <div>
          <div style={{ fontSize: 9, color: "#7a8a7d", letterSpacing: 1, marginBottom: 4, paddingLeft: 2 }}>
            BIG DRAWERS (15.3 × 12.7 × 5.8 cm)
          </div>
          {BIG_ROWS.map((row) => (
            <div key={row} style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 4, marginBottom: 4 }}>
              {BIG_COLS.map((col) => {
                const id = `R${rackNum}-${row}-${col}`;
                const count = blockCounts[id] || 0;
                const pct = count ? Math.max(20, (count / maxCount) * 100) : 0;
                return (
                  ...
                  >
                    ...
                    {count > 0 && (
                      <div style={{ fontSize: 9, color: "#7aaa7d" }}>{count} item{count > 1 ? "s" : ""}</div>
                    )}
                    {count > 0 && (
                      <div style={{
                        position: "absolute", bottom: 0, left: 0,
                        height: 3, width: `${pct}%`, background: "#c8f04a",
                      }} />
                    )}
                    {/* drawer handle */}
                    <div style={{
                      position: "absolute", bottom: 6, right: 6,
                      width: 14, height: 3, background: "#2a2f2d",
                      borderRadius: 1,
                    }} />
                  </div>
                );
...
function PartModal({ part, onSave, onClose }) {
  const [form, setForm] = useState({
    ...
    block_location: part?.block_location || ALL_BLOCKS[0],
    notes: part?.notes || "",
  });
